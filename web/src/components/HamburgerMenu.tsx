...
export default function HamburgerMenu({ isOpen, onClose }: HamburgerMenuProps) {
  ...
  const renderCategory = (
    id: string,
    options?: { hidePill?: boolean; tagsStart?: number; tagsEnd?: number }
  ) => {
    const category = allCategories.find((c) => c.id === id);
    if (!category) return null;

    const tags = category.tags
      ? category.tags.slice(options?.tagsStart || 0, options?.tagsEnd)
      : [];
    const hasVisibleSubtags = !category.hideSubTags && tags.length > 0;

    return (
      <div
        key={`${id}-${options?.tagsStart || 0}`}
        className={`hamburger-menu__category-card hamburger-menu__category-card--${category.id} ${!hasVisibleSubtags ? "hamburger-menu__category-card--no-subtags" : ""} ${options?.hidePill ? "hamburger-menu__category-card--no-pill" : ""}`}
      >
        {!options?.hidePill && (
          <a
            href={`/przepisy?category=${category.id}`}
            className="hamburger-menu__pill"
            onClick={onClose}
          >
            {category.icon && (
              <span className="hamburger-menu__pill-icon">
  ...
  return (
    <div className={`hamburger-menu ${isOpen ? "is-open" : ""}`} aria-hidden={!isOpen}>
      ...
      <nav className="hamburger-menu__content" aria-label="Menu kategorii" ref={menuRef}>
        <div className="hamburger-menu__top-bar">
          <div className="hamburger-menu__top-bar-inner">
            ...
            <div className="hamburger-menu__right-actions">
              <div className="hamburger-menu__theme-mobile">
                <ThemeToggle />
              </div>
        ...
        <div className="hamburger-menu__mega-grid">
          ...
          <div className="hamburger-menu__mega-section hamburger-menu__mega-section--sweet">
            ...
            <div className="hamburger-menu__cols">
              <div className="hamburger-menu__col">{renderCategory("cakes", { tagsEnd: 8 })}</div>
              <div className="hamburger-menu__col">
